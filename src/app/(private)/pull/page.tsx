    ...
    <main className="relative min-h-screen bg-[#0a0a0a] p-8 overflow-hidden flex flex-col items-center justify-center">
      ...
      <div className="max-w-7xl mx-auto relative z-10 w-full flex flex-col items-center">
        {packState === "idle" && (
          <div className="text-center animate-in fade-in zoom-in duration-500">
            ...
            >
              Get New Pack
              <div className="absolute inset-0 bg-linear-to-r from-cyan-400 via-transparent to-purple-500 opacity-0 group-hover:opacity-20 transition-opacity"></div>
            </button>
          </div>
        )}

        {(packState === "fetching" || packState === "sealed" || packState === "tearing") && (
          <div className="relative w-80 h-112.5 cursor-pointer group" onClick={handleOpenPack}>
            {/* Sealed Pack Visual */}
            <div
              className={`absolute inset-0 z-20 transition-all duration-500 ${packState === "tearing" ? "pointer-events-none" : ""}`}
            >
              {/* Top Half */}
              <div
                className={`absolute top-0 left-0 w-full h-1/2 bg-zinc-800 clip-crimped border-4 border-zinc-700 shadow-2xl flex items-end justify-center pb-4 transition-all ${packState === "tearing" ? "animate-pack-tear-top" : "group-hover:-translate-y-2"}`}
              >
                <span className="text-zinc-500 font-black uppercase tracking-widest text-2xl opacity-20 select-none">
                  WRESTLER
                </span>
              </div>
              {/* Bottom Half */}
              <div
                className={`absolute bottom-0 left-0 w-full h-1/2 bg-zinc-800 clip-crimped border-4 border-zinc-700 shadow-2xl flex items-start justify-center pt-4 transition-all ${packState === "tearing" ? "animate-pack-tear-bottom" : "group-hover:translate-y-2"}`}
              >
                <span className="text-zinc-500 font-black uppercase tracking-widest text-2xl opacity-20 select-none">
                  TCG
                </span>
              ...
              {/* Central Seal/Logo */}
              <div
                className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-zinc-900 border-4 border-zinc-600 rounded-full z-30 flex items-center justify-center shadow-2xl transition-all ${packState === "tearing" ? "scale-0 opacity-0 rotate-180" : "group-hover:scale-110 group-hover:rotate-12"}`}
              >
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-8 w-full">
              {pulledCards.map((card, idx) => {
                ...
                return (
                  <div
                    key={`${card.id}-${idx}`}
                    className={`relative flex flex-col h-95 rounded-2xl border-2 transition-all duration-500 hover:-translate-y-2 ${styles.bg} ${styles.border} ${styles.glow} ${isVisible ? "animate-card-reveal opacity-100" : "opacity-0 scale-50"}`}
                  >
                    {/* Portrait */}
                    <div className="relative flex-1 m-3 rounded-xl overflow-hidden bg-zinc-950 border border-white/5 shadow-2xl">
                      ...
                      <div
                        className={`absolute bottom-2 right-2 px-2 py-1 rounded border border-white/10 text-[8px] font-black tracking-widest uppercase backdrop-blur-md ${styles.text}`}
                      >
                        {card.rarity}
                      </div>
                    </div>
                    ...
                    {card.rarity.toUpperCase() === "LEGENDARY" && isVisible && (
                      <div className="absolute inset-0 pointer-events-none">
                        ...
                        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-linear-to-t from-transparent via-amber-500/10 to-transparent animate-shimmer"></div>
                      </div>
                    )}
                  </div>
                );
      ...
      {packState === "revealed" && (
        <div className="fixed inset-0 pointer-events-none z-0">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-200 h-200 bg-white/2 rounded-full blur-[150px] animate-pulse"></div>
        </div>
      )}
    </main>
  );
